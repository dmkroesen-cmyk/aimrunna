...
const HERO_IMAGE_SETS = {
  triathlon: [
    ...
    { url: "./pics/IMG_7505.jpeg", pos: "50% 76%", size: "cover" },
    { url: "./pics/IMG_7505.jpeg", pos: "52% 84%", size: "118% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "54% 70%", size: "cover" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "54% 84%", size: "122% auto" },
    { url: "./pics/IMG_5985.jpeg", pos: "52% 82%", size: "112% auto" },
  ],
  running: [
    { url: "./pics/IMG_5985.jpeg", pos: "56% 72%", size: "cover" },
    { url: "./pics/IMG_7505.jpeg", pos: "50% 82%", size: "cover" },
    { url: "./pics/IMG_7505.jpeg", pos: "52% 83%", size: "118% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "53% 86%", size: "122% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "46% 88%", size: "128% auto" },
    { url: "./pics/IMG_5985.jpeg", pos: "44% 78%", size: "cover" },
  ],
  cycling: [
    { url: "./pics/IMG_5985.jpeg", pos: "46% 82%", size: "cover" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "58% 76%", size: "cover" },
    { url: "./pics/IMG_7505.jpeg", pos: "48% 86%", size: "116% auto" },
    { url: "./pics/IMG_5985.jpeg", pos: "58% 84%", size: "114% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "52% 86%", size: "120% auto" },
    { url: "./pics/IMG_7505.jpeg", pos: "54% 79%", size: "cover" },
  ],
  hyrox: [
    { url: "./pics/IMG_5985.jpeg", pos: "50% 84%", size: "116% auto" },
    { url: "./pics/IMG_7505.jpeg", pos: "52% 74%", size: "cover" },
    { url: "./pics/IMG_7505.jpeg", pos: "50% 85%", size: "122% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "44% 88%", size: "130% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "50% 80%", size: "cover" },
    { url: "./pics/IMG_5985.jpeg", pos: "46% 79%", size: "cover" },
  ],
  shape: [
    { url: "./pics/IMG_7505.jpeg", pos: "52% 84%", size: "118% auto" },
    { url: "./pics/IMG_5985.jpeg", pos: "50% 75%", size: "cover" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "48% 88%", size: "128% auto" },
    { url: "./pics/93124896-5C3F-41E0-9034-3463509696F4.jpeg", pos: "54% 85%", size: "120% auto" },
    { url: "./pics/IMG_5985.jpeg", pos: "50% 81%", size: "cover" },
...
const I18N = {
  de: {
    ...
    stats_elevation_gain: "Höhenmeter",
  ...
  en: {
    ...
    stats_elevation_gain: "Elevation",
...
document.getElementById("export-data-btn")?.addEventListener("click", () => {
  const account = getCurrentAccount();
  if (!account) return;
  const data = { activities: account.activities || [], plans: account.plans || [], settings: account.settings || {} };
...
document.getElementById("delete-data-btn")?.addEventListener("click", () => {
  const account = getCurrentAccount();
  if (!account) return;
  if (confirm("Alle Trainingsdaten unwiderruflich löschen?")) {
    account.activities = [];
    persistStore();
    renderProfileStats(account);
    renderStatisticsView(account);
  }
});
...
function formatRelativeDate(date) {
  ...
  if (diffDays === 0 && date.toDateString() === now.toDateString()) return "Heute";
  if (diffDays <= 1 && date.toDateString() !== now.toDateString()) return "Gestern";
...
function buildActivityCardHtml({ item, actorEmail, actorName = "", actorAvatar, actorPoints = 0, viewerEmail = "", showPropsAction = false }) {
  ...
  let propsHtml = "";
  if (showPropsAction && viewerEmail && viewerEmail !== actorEmail) {
    propsHtml = `<div class="activity-card-actions"><button type="button" class="activity-card-props-btn ${hasPropd ? "is-active" : ""}" data-props-owner="${escapeHtml(actorEmail)}" data-props-activity="${escapeHtml(item.id)}"><svg viewBox="0 0 20 20" width="14" height="14"><path d="M10 1.5l2.4 5.2 5.6.5-4.2 3.7 1.3 5.6L10 13.8l-5.1 2.7 1.3-5.6L2 7.2l5.6-.5z" fill="currentColor"/></svg> Props${propsCount > 0 ? ` (${propsCount})` : ""}</button></div>`;
  } else if (propsCount > 0) {
    propsHtml = `<div class="activity-card-actions"><span class="activity-card-props-count"><svg viewBox="0 0 20 20" width="12" height="12"><path d="M10 1.5l2.4 5.2 5.6.5-4.2 3.7 1.3 5.6L10 13.8l-5.1 2.7 1.3-5.6L2 7.2l5.6-.5z" fill="currentColor"/></svg> ${propsCount}</span></div>`;
  }
...
function renderVolumeChart(allActivities, range, metric) {
  const svg = document.getElementById("stats-volume-svg");
  if (!svg) return;
  metric = metric || currentStatsMetric;
  ...
  activities.forEach((a) => {
    ...
    if (a.sportType === "run") months[key].run += val;
    else if (a.sportType === "bike") months[key].bike += val;
  ...
  const totalHours = activities.reduce((s, a) => s + (Number(a.movingTimeSec) || 0), 0) / 3600;
  ...
  const totalElevation = activities.reduce((s, a) => s + (Number(a.elevationGainM) || 0), 0);
  const streak = computeStreak(activities);

  if (useHours) {
    const avgWeeklyH = totalHours / Math.max(1, weeks);
    setText(document.getElementById("stats-total-km"), `${totalHours.toFixed(1)} h`);
  ...
  setText(document.getElementById("stats-longest-streak"), `${streak} d`);
}
...
// Stats metric toggle (km / hours)
document.addEventListener("click", (e) => {
  const btn = e.target.closest(".stats-metric-btn");
  if (!btn) return;
  const metric = btn.dataset.statsMetric;
  if (!metric) return;
...
function renderHealthMetrics(account) {
  ...
  // Resting HR
  const rhrEl = document.getElementById("health-rhr-value");
  if (rhrEl) {
    const rhr = profile.restingHr || account?.settings?.inputData?.restingHr;
    rhrEl.textContent = rhr || "--";
  }

  // RHR chart
  renderHealthTrendChart("health-rhr-chart", history, "restingHr", {
    color: "#E05252",
    label: "Ruhe-HF",
    unit: "bpm",
    fallbackValue: profile.restingHr
  });

  // HRV
  const hrvEl = document.getElementById("health-hrv-value");
  if (hrvEl) {
    const hrv = history.length > 0 ? history[history.length - 1]?.hrv : null;
    hrvEl.textContent = hrv || "--";
  }

  renderHealthTrendChart("health-hrv-chart", history, "hrv", {
    color: "#8E7CC3",
    label: "HRV",
    unit: "ms",
    fallbackValue: null
  ...
  // Sleep
  const sleepEl = document.getElementById("health-sleep-value");
  if (sleepEl) {
    const lastSleep = history.length > 0 ? history[history.length - 1]?.sleepHours : null;
    sleepEl.textContent = lastSleep ? lastSleep.toFixed(1) : "--";
  }

  renderHealthTrendChart("health-sleep-chart", history, "sleepHours", {
    color: "#5B9BD5",
    label: "Schlaf",
    unit: "h",
    fallbackValue: null
  ...
  // Recovery
  const recoveryEl = document.getElementById("health-recovery-value");
  if (recoveryEl) {
    const recovery = calculateRecoveryScore(account);
    const score = recovery?.score;
    recoveryEl.textContent = score != null ? Math.round(score) : "--";
    if (score != null) {
      if (score >= 67) recoveryEl.style.color = "#4CAF82";
      else if (score >= 34) recoveryEl.style.color = "#E5A93D";
      else recoveryEl.style.color = "#E05252";
    }
  }

  renderHealthTrendChart("health-recovery-chart", history, "recovery", {
    color: "#4CAF82",
    label: "Recovery",
    unit: "%",
    fallbackValue: null
  ...
  // 7-day averages
  const avg7Rhr = document.getElementById("health-rhr-avg7");
  if (avg7Rhr && history.length > 0) {
    const last7 = history.slice(-7).filter(h => h.restingHr).map(h => h.restingHr);
    avg7Rhr.textContent = last7.length > 0 ? Math.round(last7.reduce((a, b) => a + b, 0) / last7.length) + " bpm" : "--";
  }

  const avg7Sleep = document.getElementById("health-sleep-avg7");
  if (avg7Sleep && history.length > 0) {
    const last7 = history.slice(-7).filter(h => h.sleepHours).map(h => h.sleepHours);
    avg7Sleep.textContent = last7.length > 0 ? (last7.reduce((a, b) => a + b, 0) / last7.length).toFixed(1) + " h" : "--";
  }
}

function renderHealthTrendChart(containerId, history, key, opts) {
  const el = document.getElementById(containerId);
  if (!el) return;

  const data = history.filter(h => h[key] != null).slice(-30);

  if (data.length < 2) {
    el.innerHTML = '<div class="health-chart-empty">Daten werden mit der Zeit erfasst</div>';
    return;
  }
  ...
  const points = data.map((d, i) => ({
    ...
    y: H - 10 - ((d[key] - minV) / range) * (H - 20)
  }));

  const pathD = points.map((p, i) => `${i === 0 ? "M" : "L"}${p.x},${p.y}`).join(" ");
  const areaD = `M${points[0].x},${H} ${pathD} L${points[points.length - 1].x},${H} Z`;

  el.innerHTML = `
    <svg viewBox="0 0 ${W} ${H}" class="health-trend-svg">
      <defs>
      ...
      <path d="${pathD}" fill="none" stroke="${opts.color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      <circle cx="${points[points.length - 1].x}" cy="${points[points.length - 1].y}" r="3" fill="${opts.color}"/>
    </svg>
  `;
}
